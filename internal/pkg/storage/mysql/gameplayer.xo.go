...
func (s *GamePlayerStorage) Insert(ctx context.Context, item GamePlayer) (int, error) {
	...
	res, err := s.db.ExecContext(ctx, sqlstr, item.FkGameID, item.FkUserID, item.RegisteredBy, item.Degree)
	if err != nil {
		return 0, err
	}
	// retrieve id
...
func (s *GamePlayerStorage) Update(ctx context.Context, item GamePlayer) error {
	...
	if _, err := s.db.ExecContext(ctx, sqlstr, item.FkGameID, item.FkUserID, item.RegisteredBy, item.Degree, item.ID); err != nil {
		return err
	}
...
func (s *GamePlayerStorage) Upsert(ctx context.Context, item GamePlayer) error {
	...
	if _, err := s.db.ExecContext(ctx, sqlstr, item.ID, item.FkGameID, item.FkUserID, item.RegisteredBy, item.Degree); err != nil {
		return err
	}
...
func (s *GamePlayerStorage) GetGamePlayerByFkUserID(ctx context.Context, fkUserID sql.NullInt64) ([]*GamePlayer, error) {
	...
	rows, err := s.db.QueryContext(ctx, sqlstr, fkUserID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *GamePlayerStorage) GetGamePlayerByID(ctx context.Context, id int) (*GamePlayer, error) {
	...
	logger.Debugf(ctx, sqlstr, id)
	gp := GamePlayer{}
	if err := s.db.QueryRowContext(ctx, sqlstr, id).Scan(&gp.ID, &gp.FkGameID, &gp.FkUserID, &gp.RegisteredBy, &gp.Degree, &gp.CreatedAt, &gp.DeletedAt); err != nil {
		return nil, err
	}
	return &gp, nil
}
