...
func TestRegistrator_UpdatePayment(t *testing.T) {
	t.Run("internal error while get game", func(t *testing.T) {
		...
		fx.gamesFacade.EXPECT().UpdatePayment(fx.ctx, int32(1), int32(2)).Return(errors.New("some error"))
	...
	t.Run("ok", func(t *testing.T) {
		...
		fx.gamesFacade.EXPECT().UpdatePayment(fx.ctx, int32(1), int32(2)).Return(nil)
